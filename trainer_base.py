...
class TrainerBase(L.LightningModule):
    def __init__(
        ...
        if vocab_size is None:
            if hasattr(self.config.data, 'vocab_file'): 
                self.vocab_size = len(self.tokenizer.added_tokens_decoder)
        ...
        elif self.config.algo.backbone == "cont_dit": 
            backbone = models.dit_cont.ContDIT(self.config.model, vocab_size=self.vocab_size, mixed_coeff=self.config.algo.mixed_coeff)
        ...
        if 'state_dict' in checkpoint:
            ...
            for key, value in state_dict.items():
                if "teacher" not in key: 
                    cleaned_key = key
                    # Remove _orig and other modifiers
...
class Diffusion(TrainerBase):
    ...
    @torch.no_grad()
    def generate_samples(self, num_samples, num_steps=None, eps=1e-5, prompt_mask=None, prompt_tokens=None):
        """Generate samples from the model."""
        # Lightning auto-casting is not working in this method for some reason
        ...
        if prompt_tokens is not None and prompt_mask is not None:
            ...
            # assumes that they will be the same shape
        ...
        for i in range(num_steps):
            ...
            if self.sampler == "ancestral":
                # for mdlm, because of SUBS, the prompt mask wont be used 
                # but is needed to make duo / usdms condition on promtps / infill

                _, x = self._ancestral_update(x=x, t=t, dt=dt, p_x0=None, prompt_mask=prompt_mask)
        ...
        if self.config.sampling.noise_removal == "ancestral":
            if self.sampler == "analytic":
                ...
                _, x = self._ancestral_update(
                    x=x, t=t0, dt=None, p_x0=p_x0_cache, noise_removal_step=True, prompt_mask=prompt_mask
                )
...
class AbsorbingState(Diffusion):
    def __init__(self, config, tokenizer):
        ...
        if hasattr(config.data, 'vocab_file'):
            vocab_size = len(tokenizer.added_tokens_decoder)
            self.token_offset = True
        else:
            self.token_offset = False
    ...
    def _ancestral_update(self, x, t, dt, p_x0=None, noise_removal_step=False, **kwargs):
        ...
        assert alpha_t.ndim == 2
        p_x0 = (self.forward(x, self._sigma_from_alphat(alpha_t)).double())
        p_x0 = p_x0.exp()
