...
def _apply_rotary_emb_torch(x, cos, sin):
    """Pure-PyTorch rotary embedding (replaces flash_attn.layers.rotary.apply_rotary_emb_torch)."""
...
def apply_rotary_pos_emb(qkv, cos, sin):
    ...
    # Fallback: apply to Q and K in the packed QKV (B, S, 3, H, D)
    q = _apply_rotary_emb_torch(qkv[:, :, 0], cos, sin)
    k = _apply_rotary_emb_torch(qkv[:, :, 1], cos, sin)
    qkv[:, :, 0] = q
    qkv[:, :, 1] = k
    return qkv
...
class DDiTBlockCausal(nn.Module):
    ...
    def forward(self, x, rotary_cos_sin, **kwargs):
        ...
        if HAS_FLASH_ATTN:
            ...
            qkv = einops.rearrange(qkv, "(b s) three h d -> b s three h d", b=batch_size)
...
# Assumes that the vocab size accounts for the mask token 
class ContDIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, config, vocab_size: int, mixed_coeff: float=.5):
        super().__init__()
        if type(config) == dict:
        ...
        for _ in range(config.n_blocks):
            if self.causal:
                block = DDiTBlockCausal(
                    dim=dim, n_heads=config.n_heads, dropout=config.dropout
                )
            else:
                block = DDiTBlock(
                    dim=dim,
    ...
    # clean mask: indicates which positions are clean with 1, which are corrupted with 0
    # def forward(self, x, discrete_noise):
    def forward(self, xt, discrete_noise, reveal_mask, continuous_noise, embedding=None, ret_embedding=False, is_embed=False):
        # sigma = discrete_noise
        # x = self.vocab_embed(x)
        x = xt 
        sigma = -1 * torch.log(1 - discrete_noise)
        c_in = 1 / (1 + continuous_noise ** 2) ** .5
        # c_in = torch.ones_like(clean_mask, device=c_in.device)* clean_mask * (1/self.scaling_factor)+ (1 - clean_mask) * c_in[:, None]
        # # pdb.set_trace()
        if is_embed == False:
            x = self.vocab_embed(x)
